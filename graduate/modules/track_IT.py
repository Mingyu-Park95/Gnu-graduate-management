    ...
    if eduYear <2019:
        ...
        for db_Advance in db_Advance_list:
            if db_Advance in user_Take_list:
                user_takeAdvance_point += Track.objects.get(Q(lectureNum=db_Advance) & Q(eduYear__lt=2019) & Q(trackName ="IT융합시스템개발") & Q(seperate="트랙심화")).lecturePoint
        ...
        for db_Basic in db_Basic_list:
            if db_Basic in user_Take_list:
                user_takeBasic_point += Track.objects.get(
                    Q(lectureNum=db_Basic) & Q(eduYear=2019) & Q(trackName="IT융합시스템개발") & Q(seperate="트랙기본")).lecturePoint
        ...
        for db_Advance in db_Advance_list:
            if db_Advance in user_Take_list:
                user_takeAdvance_point += Track.objects.get(
                    Q(lectureNum=db_Advance) & Q(eduYear=2019) & Q(trackName="IT융합시스템개발") & Q(seperate="트랙심화")).lecturePoint
