...
def Integration_Judge(userName,eduYear,studentMajor): #이수여부 판단
    ...
    if len(noTake_Num_list) > 2: # 듣지 않은 수강과목 수가 2개보다 많다면 듣지 않은 영역 표시
        for noTake in noTake_Num_list:
            resultValue += IntegrationList.objects.get(lectureNum=noTake).lectureName
            resultValue += "/"
        return resultValue + "중 {0} 개 이상 영역을 이수 하셔야 합니다.".format(len(noTake_Num_list)-2)

    else:
        resultValue +="통합교양을 이수하셨습니다."
        return resultValue
        ...
        if db_list[1] not in noTake_list and db_list[2] not in noTake_list:  # 대학영어 1,2 를 이수했을때
            if (db_list[3] not in noTake_list or db_list[6] not in noTake_list):  # 인사글을 들었고
                if count >=1:  # 비사, 인성, 수통 중 하나라도 들었다면
                    result_value+= "공통(역량) 교양을 이수 하셨습니다."
                    return result_value
                else:
                    result_value+="{0} 중 1개를 이수 하셔야 합니다.".format(select_str)
                    return result_value
            else:  # 인사글 안들었다면
                if count >=1:  # 비사, 인성, 수통 중 하나라도 들었다면
                    result_value+="{0}을 이수하셔야 합니다".format(db_list[3])
                    return result_value
                else:
                    result_value+="{0}을 이수하셔야 하고 {1} 중 1개를 이수 하셔야 합니다.".format(db_list[3],select_str)
                    return result_value
            ...
            if (db_list[3] not in noTake_list or db_list[6] not in noTake_list):  # 인사글을 들었고
                if count >=1:  # 비사, 인성, 수통 중 하나라도 들었다면
                    result_value+="{0}을 이수하셔야 합니다.".format(db_list[1])
                    return result_value
                else:
                    result_value+="{0}을 이수하셔야 하고 {1} 중 1개를 이수 하셔야 합니다.".format(db_list[1],select_str)
                    return result_value
            else:  # 인사글 안들었다면
                if count >=1:  # 비사, 인성, 수통 중 하나라도 들었다면
                    result_value+="{0}과 {1}을 이수하셔야 합니다".format(db_list[1],db_list[3])
                    return result_value
                else:
                    result_value+="{0}과 {1}을 이수하셔야 하고 {2} 중 1개를 이수 하셔야 합니다.".format(db_list[1],db_list[3], select_str)
                    return result_value

        elif db_list[1] not in noTake_list and db_list[2] in noTake_list: # 대학영어2 안들었을 때
            if (db_list[3] not in noTake_list or db_list[6] not in noTake_list):  # 인사글을 들었고
                if count >=1:  # 비사, 인성, 수통 중 하나라도 들었다면
                    result_value+="{0}을 이수하셔야 합니다.".format(db_list[2])
                    return result_value
                else:
                    result_value+="{0}을 이수하셔야 하고 {1} 중 1개를 이수 하셔야 합니다.".format(db_list[2],select_str)
                    return result_value
            else:  # 인사글 안들었다면
                if count >=1:  # 비사, 인성, 수통 중 하나라도 들었다면
                    result_value+="{0}과 {1}을 이수하셔야 합니다".format(db_list[2],db_list[3])
                    return result_value
                else:
                    result_value+="{0}과 {1}을 이수하셔야 하고 {2} 중 1개를 이수 하셔야 합니다.".format(db_list[2],db_list[3], select_str)
                    return result_value

        else:  # 대학영어, 잉글리시존 둘다 만족 못했을 시
            if (db_list[3] not in noTake_list or db_list[6] not in noTake_list):  # 인사글을 들었고
                if count >=1:  # 비사, 인성, 수통 중 하나라도 들었다면
                    result_value+="{0} 혹은 {1}, {2} 과목을 이수 하셔야 합니다.".format(db_list[0], db_list[1], db_list[2])
                    return result_value
                else:
                    result_value+="{0} 혹은 {1}, {2} 과목을 이수 하셔야 하고 {3} 중 1개를 이수 하셔야 합니다.".format(db_list[1], db_list[2],db_list[3], select_str)
                    return result_value
            else:  # 인사글 안들었다면
                if count >=1:  # 비사, 인성, 수통 중 하나라도 들었다면
                    result_value+="{0} 혹은 {1} {2} 과목을 이수 하셔야 하고 {3} 과목을 수강하셔야 합니다.".format(db_list[0],db_list[1],db_list[2],db_list[3])
                    return result_value
                else:
                    result_value+="{0} 혹은 {1} {2} 과목을 이수, {3}과목 이수, {4} 중 1개를 이수 하셔야 합니다.".format(db_list[0],db_list[1],db_list[2], db_list[3], select_str)
                    return result_value
        ...
        if db_list[1] not in noTake_list and db_list[2] not in noTake_list:  # 대학영어를 다 들었을 때
            if (db_list[3] not in noTake_list):  # 인사글을 들었다면
                if count >=2:  # 선택과목 요건 만족 했다면
                    ...
                    result_value+="{0} 중 2개를 이수 하셔야 합니다.".format(select_str)
                    return result_value
            else:  # 인사글 안들었다면
                if count >=2:  # 선택과목 요건 만족 했다면
                    result_value+="{0}을 이수하셔야 합니다".format(db_list[3])
                    ...
                    result_value+="{0}을 이수하시고 {1} 중 2개를 이수하셔야 합니다.".format(db_list[3],select_str)
                    return result_value
            ...
            if (db_list[3] not in noTake_list):  # 인사글을 들었다면
                if count >=2:  # 선택과목 요건 만족 했다면
                    ...
                    result_value+="{0}을 이수하시고 {1} 중 2개를 이수 하셔야 합니다.".format(db_list[1],select_str)
                    return result_value
            else:  # 인사글 안들었다면
                if count >=2:  # 선택과목 요건 만족 했다면
                    result_value+="{0}과 {1}를 이수하셔야 합니다.".format(db_list[1],db_list[3])
                    return result_value
                else:
                    result_value+="{0}을 이수, {1}를 이수, {2} 중 2개를 이수 하셔야 합니다.".format(db_list[1],db_list[3],select_str)
                    return result_value
            ...
            if (db_list[3] not in noTake_list):  # 인사글을 들었다면
                if count >=2:  # 선택과목 요건 만족 했다면
                    ...
                    result_value+="{0}을 이수하시고 {1} 중 2개를 이수 하셔야 합니다.".format(db_list[2],select_str)
                    return result_value
            else:  # 인사글 안들었다면
                if count >=2:  # 선택과목 요건 만족 했다면
                    result_value+="{0}과 {1}를 이수하셔야 합니다.".format(db_list[2],db_list[3])
                    return result_value
                else:
                    result_value+="{0}을 이수, {1}를 이수, {2} 중 2개를 이수 하셔야 합니다.".format(db_list[2],db_list[3],select_str)
                    return result_value

        else:  # 대학영어 잉글리시존 둘다 안들었을 때
            if (db_list[3] not in noTake_list):  # 인사글을 들었다면
                if count >=2:  # 선택과목 요건 만족 했다면
                    result_value+="{0} 혹은 {1}, {2}을 이수하셔야 합니다.".format(db_list[0],db_list[1],db_list[2])
                    return result_value
                else:
                    result_value+="{0} 혹은 {1},{2}을 이수하고  {3} 중 2개를 이수 하셔야 합니다.".format(db_list[0],db_list[1],db_list[2],select_str)
                    return result_value
            else:  # 인사글 안들었다면
                if count >=2:  # 선택과목 요건 만족 했다면
                    result_value+="{0} 혹은 {1}, {2}을 이수하시고 {3}를 이수하셔야 합니다.".format(db_list[0],db_list[1],db_list[2],db_list[3])
                    return result_value
                else:
                    result_value+="{0} 혹은 {1},{2}을 이수하고 {3} 이수 {4} 중 2개를 이수 하셔야 합니다.".format(db_list[0],db_list[1],db_list[2],db_list[3],select_str)
                    return result_value
